...
/**
 * Create a Medical Record (role info: done by Doctor only)
 */
public class CreateMedicalRecord extends JFrame{
	
	// ACHTUNG: doctorId and date of MedicalRecord are not entered by Doctor, but created automatically here 
	// ACHTUNG: only if the Doctor clicks on prescription "YES", the medication name, start and end date of medication need to be entered
	 ...
	 */
	public CreateMedicalRecord(User user) {
		setVisible(true);
		setResizable(false);
		this.user = user;
	...
	private void initialize() {
		...
		List<User> userList = userMgmt.getUserList().stream().filter(user -> user instanceof Patient).collect(Collectors.toList());
		
		if (userList.size() != 0)
			userList.forEach(user -> cBPatientId.addItem("Id: "+user.getId()+" - Username: "+user.getUsername()+""));
		...
		btnAddDevice.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String id = ((String)cBPatientId.getSelectedItem()).toString().substring(4, cBPatientId.getSelectedItem().toString().indexOf('-')-1);
				deviceMgmt.addDevice(DeviceType.SENSOR, Integer.parseInt(id), medicalRecordID);
				JOptionPane.showMessageDialog(null,
            		    "Device added.",
            		    "Create Medical Record Information",
            		    JOptionPane.INFORMATION_MESSAGE); 
				patientMonitoringMgmt.addPatientMonitoring(Integer.parseInt(id), 
		...
		cbMedicalCondition.setModel(new DefaultComboBoxModel<MedicalCondition>(MedicalCondition.values()));
